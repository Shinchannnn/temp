...
app.post('/update-template-values', async (req, res) => {
    try {
        ...
        console.log("Existing template data:", template); // Debugging

        let updateFields = {};
        for (const [key, value] of Object.entries(updatedValues)) {
            if (key in template.toObject()) { 
                updateFields[key] = value; // Update only existing fields
            } else {
                console.log(`Skipping update for non-existing field: ${key}`); // Debugging
            }
        }
        ...
        if (updateResult.modifiedCount === 0) {
            return res.status(404).json({ error: "No matching fields found to update." });
        }
